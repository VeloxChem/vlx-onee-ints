from overlapwriter import write_overlap
...
"""
    ...
    text += f"""
        ...
        {{
            ...
            double grad_i = S_ij_00 * (
"""

    text += write_overlap(f'{a}{b}', 'gradient_a')
...
"""
    ...
    if a == b:
        W_sym_factor = '((i == j) ? Wij : (Wij + Wji))'
    else:
        W_sym_factor = '(Wij + Wji)'

    text += f"""
                    auto coef_sph = i_coef_sph * j_coef_sph;

                    auto Wij = W[i_cgto_sph * naos + j_cgto_sph];
                    auto Wji = W[j_cgto_sph * naos + i_cgto_sph];

                    double W_sym = {W_sym_factor};

                    S_grad_omp[thread_id].row(i_atom)[m] += grad_i * coef_sph * W_sym;
                    S_grad_omp[thread_id].row(j_atom)[m] -= grad_i * coef_sph * W_sym;
                }}
            }}
        }}
    }}
